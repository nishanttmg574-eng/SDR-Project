...
Original project brief. Some implementation details have evolved during the audit phases; current user-facing setup and behavior are documented in `README.md`, `QUICKSTART.md`, and `ABOUT.md`.
...
- Settings screen (workspace name, company description, tier definitions, AI provider, AI model)
- Bulk AI scoring with cancel button, progress bar, batched saves
- Single-account "Research & score" for deep dives
- JSON backup and restore as the normal backup path; raw SQLite backup requires stopping the app and copying the WAL files with `workspace.db`
...
- **Settings**: workspace name, company description, tier1 definition, lower tier definitions, AI provider, AI model

## Chosen stack

**Next.js 15 (App Router) + TypeScript + React 19 + Tailwind CSS + better-sqlite3.**
The AI layer uses official provider SDKs through a local provider abstraction: `openai` for OpenAI and `@anthropic-ai/sdk` for Anthropic.
...
- Needs to call the selected AI provider API for scoring and call prep
...
- **AI SDKs**: `openai` and `@anthropic-ai/sdk` (official)

Optimize for: simplicity, iteration speed, and "the user could explain this stack to a non-engineer."

## AI behavior rules

- Model IDs live in settings/config and route through a provider abstraction. Never hardcoded in call sites.
- API keys come from `.env` (`OPENAI_API_KEY` or `ANTHROPIC_API_KEY`). Never committed, never logged.
